...
namespace Steamless.Model
{
    ...
    internal class AutomaticPlugin : SteamlessPlugin
    {
        ...
        /// </summary>
        public override string Name => "자동 선택";

        /// <summary>
        /// Gets the description of this plugin.
        /// </summary>
        public override string Description => "지정된 파일에 적합한 플러그인을 자동으로 감지하여 언팩합니다.";
            ...
            try
            {
                ...
                if (f.Parse())
                {
                    ...
                    if (!f.HasSection(".bind"))
                    {
                        ...
                        this.Log("이 파일은 SteamStub로 패킹되지 않은 것으로 보입니다!", LogMessageType.Error);
                        this.Log("파일에 예상된 '.bind' 섹션이 없습니다!", LogMessageType.Error);
                        this.Log("", LogMessageType.Error);
                        return false;
                    }
                }
                ...
                {
                    this.Log("", LogMessageType.Error);
                    this.Log("이 파일은 올바른 Win32 PE 파일이 아닌 것 같습니다. 언팩할 수 없습니다!", LogMessageType.Error);
                    this.Log("", LogMessageType.Error);
                }
            }
            catch (Exception e)
            {
                this.Log("예외 발생으로 인해 선택한 파일을 파싱하거나 언팩하지 못했습니다:", LogMessageType.Error);
                this.Log("", LogMessageType.Error);
