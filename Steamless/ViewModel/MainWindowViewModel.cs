...
namespace Steamless.ViewModel
{
    ...
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        private async void Initialize()
        {
            ...
            this.CurrentTask = new StatusTask("초기화 중...");
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (plugin.CanProcessFile(this.InputFilePath))
                        this.AddLogMessage(this, !plugin.ProcessFile(this.InputFilePath, this.Options) ? new LogMessageEventArgs("파일 언팩에 실패했습니다.", LogMessageType.Error) : new LogMessageEventArgs("파일 언팩에 성공했습니다!", LogMessageType.Success));
                    else
                        this.AddLogMessage(this, new LogMessageEventArgs("파일 언팩에 실패했습니다.", LogMessageType.Error));
                }
                catch (Exception ex)
                {
                    this.AddLogMessage(this, new LogMessageEventArgs("파일 언팩 시도 중 처리되지 않은 예외가 발생했습니다.", LogMessageType.Error));
                    this.AddLogMessage(this, new LogMessageEventArgs("예외:", LogMessageType.Error));
